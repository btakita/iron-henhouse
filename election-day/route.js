...
import {
    route, fn$route as route$lib__fn$route, assign__route$$
}
from "ctx-core/route/lib";
import {
    ctx$reset
}
from "site/lib";
import {
    assign__voting_issue$$
}
from "voting-issue/lib";
import {
    log, debug
}
...
export
function assign__election_day$route$$() {
...
function fn$route(ctx, ...ctx$rest$$) {
    return route$lib__fn$route(ctx, {
        fn$route$ctx: fn$ctx
    }, ...ctx$rest$$);
}
